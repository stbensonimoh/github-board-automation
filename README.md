...
Images are pinned by digest so local runs match CI exactly. The first command exits 1 with no matches while the scaffold has no shell files; that is expected until #4 lands. CI runs the same three checks on every PR. See `SPEC.md` for the full spec.
