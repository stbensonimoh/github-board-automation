...
open_test_pr() { # ISSUE_NUMBER LABEL -> creates a branch with one commit
  local issue="$1" label="$2" branch stamp base_sha
  stamp=$(date +%s)
  branch="e2e/$label-$stamp"
  ...
  # the file path is unique per run: a merged earlier run leaves the old
  # file on main, and an update PUT would need its sha
  gh api --method PUT "repos/$REPO/contents/.e2e-$label-$stamp" \
    -f message="e2e: $label" \
    -f content="aGk=" \
